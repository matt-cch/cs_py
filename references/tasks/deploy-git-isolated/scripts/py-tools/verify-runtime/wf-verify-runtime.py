...
用法：
    python wf-verify-runtime.py --devroot "D:\\pjt\\vscode\\vsc_py"
...
def main():
    ...
    for i, tool in enumerate(tools, 1):
        ...
        if local_result.get("exe_exists") and upstream_cfg.get("enabled", True) and upstream_cfg.get("query_param"):
            ...
            print(f"  [query] upstream={upstream_result.get('upstream_version') or 'N/A'}, source={upstream_result.get('upstream_source') or 'N/A'}")
        ...
        if local_result.get("exe_exists"):
            compare_result = run_atomic(
                ...
                ["--local-ver", local_result.get("local_version", ""),
                 "--upstream-ver", upstream_result.get("upstream_version", "")]
            )
