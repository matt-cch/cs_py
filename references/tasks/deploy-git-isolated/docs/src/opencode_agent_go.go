...
// OpenCode CLI Agent 核心实现
// 来源: https://github.com/opencode-ai/opencode/blob/main/internal/llm/agent/agent.go
// 获取时间: 2026-06-22
// 关键研究价值: processGeneration 展示了标准 ReAct Agent 循环的 Go 实现
// 本地快照路径: D:\pjt\cursor\cs_py\references\tasks\deploy-git-isolated\docs\src\opencode_agent_go.go
...
// processGeneration 是 OpenCode Agent 的核心 ReAct 循环。
// 关键机制:
// 1. for {} 无限循环
// 2. 每次迭代调用 streamAndHandleEvents() 完成一次 LLM 生成
// 3. 如果 LLM 返回 FinishReasonToolUse，说明需要执行工具
// 4. 把 assistantMessage + toolResults 追加到 msgHistory，continue 进入下一轮
// 5. 直到 LLM 不再请求工具（FinishReasonEndTurn 等），返回最终响应
...
// streamAndHandleEvents 完成一次完整的 LLM Stream 处理 + Tool 执行。
// 步骤:
// 1. provider.StreamResponse() 发起流式请求
// 2. processEvent() 逐事件处理（thinking/content/tool_use_start/tool_use_stop/error/complete）
// 3. 流结束后，遍历 assistantMsg.ToolCalls()，逐个调用 tool.Run()
// 4. 把所有 toolResults 打包成 message.Tool 消息返回
