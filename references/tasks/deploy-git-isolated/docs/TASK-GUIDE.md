...
| 速查表 | `SOP-CHEATSHEET.md` | 与基线定义同名，位置在 `scripts/`（因内容强绑定脚本） |
...
- **文件**：`scripts/SOP-CHEATSHEET.md`
- **ENTRY.json 登记**：`"cheatsheet": "references/tasks/deploy-git-isolated/scripts/SOP-CHEATSHEET.md"`
- **选择理由**：内容与 `scripts/*.ps1` 强绑定，放在同目录便于 Human 查命令时顺便看脚本

### 4.2 调用格式对照

详见 `scripts/SOP-CHEATSHEET.md`，核心约定：
...
| 变更 | 联动文件 | 状态 |
|------|---------|------|
| 新增 `git-*.ps1` 脚本 | `SOP-CHEATSHEET.md` 新增对照块 | ✅ 已完成 |
...
| 基线推荐 | 本 task 实际 | 差异理由 |
|---------|-------------|---------|
| `SOP-CHEATSHEET.md` 默认在 `docs/` | 实际在 `scripts/` | 内容与脚本强绑定，Human 查命令时需同时看脚本源码 |
